...
class NomenclatureTable
{
        ...
        return $table
            ...
            ->columns([
                Tables\Columns\TextColumn::make('name')
                    ...
                    ->weight('medium')
                    ->size('sm'),
                Tables\Columns\TextColumn::make('nomenclatures_count')
                    ->label('Nb nomenclatures')
                    ->badge()
                    ->color('primary'),
                Tables\Columns\TextColumn::make('debug_info')
                    ->label('Debug Info')
                    ->state(fn ($record) => 'Festival: ' . $record->name . ' | Nomenclatures: ' . $record->nomenclatures->count()),
    ...
    private static function getNomenclaturePreview($record): string
    {
        ...
        foreach ($nomenclatures as $nomenclature) {
            $paramName = $nomenclature->resolveParameter()?->code ?? 'PARAM';
    ...
    private static function getNomenclatureSummary($record): string
    {
        ...
        // Paramètre
        if ($record->resolveParameter()) {
            $param = $record->resolveParameter();
            $paramInfo = $param->category.' - '.$param->code;
    ...
    private static function getQualityScore($record): string
    {
        ...
        if ($record->is_required && ! $record->default_value) {
            $score -= 20;
        }
    ...
    private static function getFestivalNomenclatures($festivalId)
    {
        return Nomenclature::where('festival_id', $festivalId)
            ->with(['festivalParameter.parameter', 'festival'])
            ->orderBy('order_position')
            ->get();
    }
    ...
    private static function generateTestNomenclature($record, array $data): string
    {
        ...
        foreach ($nomenclatures as $nomenclature) {
            ...
            $sampleValue = self::findSampleValueForParameter($nomenclature->resolveParameter(), $sampleData);
    ...
    private static function findSampleValueForParameter($parameter, array $sampleData): string
    {
        ...
        $paramName = strtolower($parameter->code);
        ...
        return match (true) {
            ...
            default => strtoupper($parameter->code)
        };
    }
    ...
    private static function getFestivalNomenclaturesQuery()
    {
        return Festival::query()
            ...
            ->with(['nomenclatures.festivalParameter.parameter'])
    ...
    private static function buildQualityIndicators($festival): string
    {
        ...
        if ($missingRequired > 0) {
            $score -= 15;
        } // Paramètres requis sans défaut
        if ($withRules / $totalCount < 0.5) {
            $score -= 10;
        } // Peu de règles
