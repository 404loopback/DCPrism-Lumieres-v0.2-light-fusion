...
class VersionGenerationService
{
    ...
    private function detectVersionTypesFromValue(string $value): array
    {
        ...
        // Détecter VF
        if (str_contains($value, 'VF') && !str_contains($value, 'VOSTFR')) {
            $versions[] = [
                'type' => 'VF',
                'audio_lang' => 'fr',
                'sub_lang' => null
    ...
    private function detectVersionTypeFromString(string $versionString): array
    {
        ...
        if ($upper === 'VF' || str_contains($upper, 'FRANÇAIS')) {
            return ['type' => 'VF', 'audio_lang' => 'fr', 'sub_lang' => null];
        }
