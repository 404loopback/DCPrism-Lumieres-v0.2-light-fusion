...
namespace App\Services\MovieForm;

use App\Models\Movie;
use App\Models\Festival;
use App\Models\MovieParameter;
use App\Models\FestivalParameter;
use App\Filament\Builders\FormFieldBuilder;
use App\Services\Context\FestivalContextService;
...
class MovieFormService
{
    ...
    /**
     * Extract language information from version name
     */
    public function extractLanguageFromVersionName(string $versionName): ?string
    {
        $versionUpper = strtoupper($versionName);
        
        return match (true) {
            str_contains($versionUpper, 'VO') => 'original',
            str_contains($versionUpper, 'VF') => 'fr',
            str_contains($versionUpper, 'EN') => 'en',
            default => null
        };
    }

    /**
     * Extract subtitle language from version name
     */
    public function extractSubtitleLanguageFromVersionName(string $versionName): ?string
    {
        $versionUpper = strtoupper($versionName);
        
        if (!str_contains($versionUpper, 'VOST')) {
            return null;
        }
        
        return str_contains($versionUpper, 'FR') ? 'fr' : 'original';
    }
