...
class Nomenclature extends Model
{
    ...
    /**
     * Formater une valeur de paramètre sans séparateur (pour génération globale)
     */
    public function formatParameterValue($rawValue, ?Movie $movie = null): ?string
    {
    ...
    /**
     * Formater une valeur selon les règles de la nomenclature (avec séparateur pour compatibilité)
     */
    public function formatValue($rawValue, ?Movie $movie = null): ?string
    {
        $result = $this->formatParameterValue($rawValue, $movie);
        
        // Ajouter le séparateur à la fin si défini
        if ($result && ! empty($this->separator)) {
    ...
    public function getTitleAttribute(): string
    {
        ...
        if ($parameter) {
            $parts[] = $parameter->name ?? 'Paramètre';
        }
    ...
    /**
     * Résoudre le paramètre via festivalParameter
     */
    public function resolveParameter(): ?Parameter
    {
        return $this->festivalParameter?->parameter;
    }
    ...
    {
        $parameter = $this->resolveParameter();
        $value = $sampleValue ?? $parameter?->code ?? 'SAMPLE';
        return $this->formatParameterValue($value) ?? '';
    }
    
    /**
     * Accesseur pour la colonne preview dans Filament
     */
